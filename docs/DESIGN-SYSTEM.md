...
### 5.11 Executive Reporting Suite (Phase 6E)

- The Governor/Executive view is a **5-tab suite** (Overview, Programmes, Registry, Coordination,
  Coverage Map) — read-only, aggregates only, no raw PII. Use forest KPI panels for headline numbers,
  the status/badge maps, and charts per this system. **Headline = net unique beneficiaries, never gross
  registrations.** Coverage renders as **absolute-count bands**, never a percentage/index (no denominator
  loaded). Omitted panels (disability/vulnerability/poverty/rural-urban, %/index metrics, external map
  layers, meetings) are **absent, not shown empty**. See `docs/PHASE-6E-BUILD-PROMPTS.md` and
  `docs/REPORTING-METRICS-DICTIONARY.md`.

### 5.12 Dashboard & data-viz craft (applies to Phase 6, 6E, 6P and any dashboard)

Dashboards must look **intentional and executive-grade**, not a flat token dump. Load the
`frontend-design` skill for composition/polish, but **the tokens in this file always win** — the skill
governs *arrangement*, never palette. Rules:

- **Hierarchy first.** Each tab has ONE hero metric (largest, forest KPI panel), then a secondary band,
  then supporting detail. The eye should land on the single most important number first. Never render
  40 equally-loud tiles.
- **Charts derive from the tokens.** Series use the lime/mint/forest ramp + semantic colors (§2); no
  rainbow defaults, no off-palette hues. One consistent color meaning across all charts (e.g. lime =
  current/primary series). Obey "dark text on lime" in labels.
- **Data-ink ratio.** Charts inform, not decorate — no 3D, no chartjunk, minimal gridlines. Prefer the
  simplest chart that carries the point (bar = comparison, line = trend, choropleth = geography).
- **Executive-grade number formatting.** Currency as ₦3.6bn / ₦450m (not raw digits), percentages to one
  decimal, thousands separators, compact axis labels, consistent units. Nulls read as "—", never 0.
- **Rhythm & whitespace.** Consistent card sizing, gutters (`--space-*`), alignment; generous whitespace;
  group related metrics into titled cards (mono eyebrow) rather than a wall.
- **Restraint with the accent.** Lime marks the *one* thing that matters per view (hero, active series,
  alert) — not every element. Mint/paper carry the calm field; forest anchors headline panels.
- **Motion subtle** (150–200ms), respects reduced-motion; charts animate in once, never loop.
- **Responsive & accessible** (§6): legible on a boardroom screen and a tablet; AA contrast on every
  chart label and KPI; keyboard-navigable filters; a "last updated" timestamp on data views.
- **Designed empty/loading states** (skeletons, one-line direction) — never blank or spinner-only.

Net: dashboards should feel like a polished executive product built *from this palette* — not a generic
admin theme, and not a re-skin that abandons the palette.
