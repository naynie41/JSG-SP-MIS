...
| **Coordination** | Partner landscape (funders · **implementing** agencies · **delivering** agencies — different sets), funding-by-partner (**amounts for self only**), implementing-agency list with a **partner tag** on each non-government row, **PROGRAMME OVERLAP** (table + LGA map indicator); meetings + reporting-compliance = inert slots. **No data-sharing / sync panel** (v2.0, FR-RPT-08) — the connectors are the agencies' own and a funder can act on none of it | `FundingPartnerCoordinationTab.tsx` | `FundingPartnerCoordinationTab.test`, `PartnerFundingTest` |
...
| Data sharing / sync health | **Removed, not deferred** (v2.0). It is the implementing agencies' operational plumbing and a funder can act on none of it — it does not return if a sync feature lands. The MDA and state-wide coordination views keep theirs. |
